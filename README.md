...
논문 카드의 학회·저널 표시 옆에는 분야·기술 키워드를 작은 청록색 해시태그로 표시합니다. `data/publications.json`과 `data/ongoing.json`의 `researchKeywords`에서 논문별 키워드를 관리하며, 좁은 화면에서는 자연스럽게 줄바꿈합니다. Project 요약에 쓰이는 기존 `tags`와는 별도로 관리합니다.
